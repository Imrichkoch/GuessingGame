...
public class GuessingDemo {
	...
	public static void main(String[] args) throws GuessingException {
		...
		int arrayH [] = new int [numberOfGuessing]; // toto pole bude ukladat cisla ktore hadal pocitac
        ...
        for(int i = 0 ; i < arrayH.length ; i++){
			...
			switch(a){
				case 'm': max = arrayH[i];break; //ak je myslene cislo mensie ako uhadol pocitac, zaradi ho do max
				case 'v': min = arrayH[i];break; // a naopak
				case 'u': System.out.println("\nUhadol!!!\n");break hadanie; //ak uzivatel uhadne program opusti cyklus
				default:System.out.println("Neplatny znak, cakam na spravny znak:"); break ;
			}
		}
	...
	public static int randomNumber(int min, int max) {
	    ...
	    int randomNum = rand.nextInt((max - min)+1) + min; //ak si program tipol prilis male cislo a dal som ze treba vacsie, furt hadal rovnake, tak som pridal 1
